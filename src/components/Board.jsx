...
const createTilePoint = ([row, col], index, layout) => {
  const usableWidth = layout.width - (layout.paddingX * 2)
  const usableHeight = layout.height - (layout.paddingY * 2)

  const baseX = layout.paddingX + (col / (layout.cols - 1)) * usableWidth
  const baseY = layout.paddingY + (row / (layout.rows - 1)) * usableHeight

  const waveX = Math.sin(index * 0.82) * 18
  const waveY = Math.cos(index * 0.74) * 14

  return {
    x: clamp(baseX + waveX, layout.paddingX - 12, layout.width - layout.paddingX + 12),
    y: clamp(baseY + waveY, layout.paddingY - 10, layout.height - layout.paddingY + 10),
  }
}
...
export default function Board({ players, positions, currentIndex }){
  ...
  const tilePoints = useMemo(() => boardCoordinates.map((coord, index) => createTilePoint(coord, index, layout)), [boardCoordinates, layout])
  ...
  useEffect(() => {
    ...
    const updateBoardMode = () => setIsMobileBoard(mediaQuery.matches)

    updateBoardMode()
    mediaQuery.addEventListener('change', updateBoardMode)

    return () => mediaQuery.removeEventListener('change', updateBoardMode)
  }, [])

  return (
    <div className="p-1 xs:p-2 sm:p-4 md:p-8 bg-gradient-to-br from-blue-50 via-cyan-50 to-slate-50">
      <div className="w-full overflow-hidden">
        <div className="board-grid relative mx-auto" id="game-board">
          ...
          <div className="board-card-area absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-0">
            <div className="board-card-stack relative">
              {/* Cartas empilhadas */}
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={`card-${i}`}
                  className="board-card absolute bg-gradient-to-br from-blue-600 to-blue-800 rounded-lg border-2 border-white shadow-lg"
                  style={{
          ...
          <svg className="board-path" viewBox={`0 0 ${layout.width} ${layout.height}`} preserveAspectRatio="none">
            <path d={trailPath} className="trail-ribbon-bg" />
            <path d={trailPath} className="trail-ribbon-dashed" />
          </svg>
          {boardCoordinates.map((_,index)=>{
            ...
            const isOccupied = Boolean(playersByTile[index])
          ...
          {players.map((p,i)=>{
            ...
            const centerXPercent = (point.x / layout.width) * 100
            const centerYPercent = (point.y / layout.height) * 100
